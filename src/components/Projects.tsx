...
export const Projects = () => {
  ...
  const updateCardDOM = (idx: number) => {
    ...
    const tp = i18n[langRef.current].projects
    ...
    const badgeText = tp.badges[idx]
    if (badge) {
      badge.textContent = badgeText ?? ''
      badge.style.display = badgeText ? 'inline-flex' : 'none'
    }
    if (num) num.textContent = `${tp.label} ${String(idx + 1).padStart(2, '0')}`
    if (title) title.textContent = project.title
    if (desc) desc.textContent = tp.descriptions[idx]
    if (tagsEl) {
      tagsEl.innerHTML = project.tags
        .map((t) => `<span class="px-2.5 py-1 text-[11px] font-mono border border-white/[0.08] text-white/35 bg-white/[0.03] hover:border-purple-600/30 hover:text-white/55 transition-all duration-200 cursor-default tracking-wider uppercase">${t}</span>`)
        .join('')
    }
  ...
  useEffect(() => {
    ...
    updateCardDOM(currentRef.current)
  }, [lang])
  ...
  // Spotlight + parallax — RAF-throttled
  ...
  return (
    <section id="projetos" ref={sectionRef} className="relative py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-5 md:px-10">

        {/* Header */}
        <div className="mb-10">
          <div className="projects-badge flex items-center gap-3 mb-4">
            <span className="font-mono text-[9px] tracking-[0.12em] text-white/22 border border-white/[0.07] px-2 py-0.5">04-25</span>
          ...
          <h2 className="projects-title font-display uppercase text-5xl md:text-6xl leading-none tracking-wide">
            {t.h2a}{' '}
        ...
        >
          <div
            className="shine-border relative border border-white/[0.08] bg-white/[0.02] overflow-hidden lg:h-[460px]"
          >
            {/* Spotlight */}
            <div
              className="proj-spotlight absolute inset-0 pointer-events-none z-10 transition-opacity duration-300"
              style={{ opacity: 0 }}
            />
            {/* Shine top */}
            <div
            ...
            <div className="flex flex-col lg:flex-row lg:h-full">
              ...
              <div className="relative overflow-hidden bg-[#0d0d0d] lg:w-[58%] aspect-video lg:aspect-auto flex-shrink-0">
                ...
                <div className="absolute top-4 left-4 z-10">
                  <span
                    className="proj-badge-pill inline-flex items-center gap-1.5 px-3 py-1 text-[10px] font-mono font-medium bg-purple-600/80 backdrop-blur-sm text-white border border-purple-400/30 tracking-wider uppercase"
                    style={{ display: t.badges[0] ? 'inline-flex' : 'none' }}
                  >
              ...
              {/* Content side */}
              <div className="flex flex-col justify-center flex-1 p-5 md:p-7 lg:p-10 lg:pl-8">
                <p
                ...
                <div className="proj-tags flex flex-wrap gap-1.5 mb-8">
                  {p0.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2.5 py-1 text-[11px] font-mono border border-white/[0.08] text-white/35 bg-white/[0.03] hover:border-purple-600/30 hover:text-white/55 transition-all duration-200 cursor-default tracking-wider uppercase"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex gap-2.5">
                  <a
                    ...
                    className="proj-link-live flex items-center gap-2 px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-mono text-[10px] tracking-[0.15em] uppercase font-medium transition-all duration-200 active:scale-95 hover:-translate-y-0.5"
                  >
                    {t.view} <ExternalIcon />
                  </a>
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="proj-link-gh flex items-center gap-2 px-5 py-2.5 border border-white/12 hover:border-purple-600/50 hover:bg-purple-600/10 text-white/65 hover:text-white font-mono text-[10px] tracking-[0.15em] uppercase font-medium transition-all duration-200 active:scale-95 hover:-translate-y-0.5"
                  >
                    GitHub <GithubIcon />
                  </a>
                </div>
        ...
        <div className="flex items-center justify-between mt-5">
          ...
          <div className="flex items-center gap-2">
            {projects.map((_, i) => (
              <button
                ...
                className="proj-dot transition-all duration-300"
                style={{
                  width: i === 0 ? '28px' : '6px',
                  height: '6px',
          ...
          <div className="flex gap-2">
            <button
              ...
              className="w-10 h-10 border border-white/[0.1] bg-white/[0.03] hover:border-purple-500/50 hover:bg-purple-600/12 text-white/45 hover:text-purple-300 flex items-center justify-center transition-all duration-200 active:scale-90"
            >
